...
def classify_prompt(l: list[str]) -> str:
    return f"""
        <s>[INST]
    Формируй ответы только на русском языке.
    ...
    Перед тобой список вопросов:
['В чем разница между лекциями и семинарами?',
 'Как мне получить диплом?',
 'Могу ли я получить удостоверение, если я не имею высшего образования?',
 'Какие документы мне предоставят после того, как я закончу программу обучения?',
 'Сколько времени обычно занимает появление записи после окончания вебинара?',
 'В какое время проходят уроки?',
 'Я узнал, что ваша школа обещает помощь с трудоустройством. Можете ли вы подробнее рассказать, что именно включает в себя эта помощь?',
 'Где я могу найти опцию для активации уведомлений о занятиях?',
 'Как мне прикрепить ссылку на работу, если я использовал git или гугл-документ?',
 'Как проходит обучение?',
 'Что нужно сделать, чтобы добавить данные о себе в профиль?',
 'Мне надо проходить отдельные курсы?',
 'Как я могу сдать домашнее задание на проверку?',
 'Будут ли уроки доступны для просмотра позже, если я их пропущу?',
 'Могу ли я пересдать свою работу?',
 'Что такое лекция/семинар/консультация/практикум?',
 'Что я получу в результате окончания обучения?',
 'Не могу прийти на вебинар, что делать?',
 'Каков процесс получения документа об окончании обучения?',
 'Какие сущности входят в основное обучение в ЛК?',
 'На какой площадке проходят уроки в GB?',
 'Как эксперт будет получать доступ к моей работе для проверки?',
 'Как проходят занятия: онлайн или в записи?',
 'Какие требования к участникам устанавливаются для программ профессиональной переподготовки и программ повышения квалификации?',
 'Как строится программа обучения?',
 'Сколько будет длится моя программа обучения?',
 'Мне бы хотелось узнать, какие программные требования необходимы для специализации Тестировщик?',
 'Есть ли у вас услуги по поддержке студентов в поиске работы после прохождения курсов?',
 'Что входит в курсы смешанного формата?',
 'Из чего состоит программа обучения?',
 'Как включить уведомления об уроках?',
 'Какие требования ПО нужны, если я учусь на программиста?',
 'Как проходят видеокурсы?',
 'Когда мне пришлют диплом?',
 'Какой срок действия моего обучения и как это влияет на количество переводов?',
 'Как получить удостоверение?',
 'Могу ли я получить диплом, если я не имею высшего образования?',
 'Я проживаю в другом регионе. Смогу ли я получить диплом или удостоверение по почте?',
 'Не смог прийти на вебинар, что делать?',
 'Если я сдам только итоговую аттестацию без промежуточных, смогу ли я получить документ об окончании курсов?',
 'Как мне активировать уведомления о предстоящих уроках?',
 'У меня нет возможности прийти на вебинар, можно ли его посмотреть в записи?',
 'Чтобы получить итоговый документ, мне нужно сдать все промежуточные аттестации или только итоговую?',
 'Где я могу найти информацию о периодах каникул на платформе GeekBrains?',
 'Какой лимит переводов на весь период обучения?',
 'Я не хочу проходить курсы во вкладке "Отдельные", это возможно?',
 'Что делать, если мои документы от вашей школы не принимаются налоговой службой?\n',
 'Какова структура программы помощи с трудоустройством вашей школы? Какие этапы или этапы включены в этот процесс?',
 'Чем программа профессиональной переподготовки отличается от программы повышения квалификации?',
 'Когда в GB пройдут каникулы в 2024 году?',
 'Где сдать ДЗ в личном кабинете?',
 'Я могу получить налоговый вычет? Какие документы нужны для получения налогового вычета?',
 'Я студент, изучающий специализацию "Программист" на вашей платформе. Можете ли вы подсказать, какие программы мне следует установить для эффективного обучения?',
 'Где на платформе я могу увидеть свой текущий учебный план?',
 'Куда обратиться, если налоговая служба не признает договор об обучении, предоставленный вашей школой?\n\n',
 'Где найти курсы, которые я буду проходить?',
 'Как мне найти курсы, на которые я записан?',
 'Как отключить уведомления об уроках для моего аккаунта?',
 'Можно ли получить налоговый вычет по вашим образовательным программам?',
 'В моей программе уроки онлайн или в записи?',
 'Где находится функция для настройки оповещений о занятиях в личном кабинете?',
 'Могу ли я перевестись в другую группу в рамках программы?',
 'Как долго я буду учиться?',
 'Какие документы мне нужно предоставить для получения удостоверения?',
 'Что делать, если налоговая не принимает договор на обучение от вашей школы?\n',
 'Что в себя включает основной блок обучения в ЛК?',
 'В чем разница между программами "Мастер" и "Про"?',
 'Что нужно сделать, чтобы воспользоваться вашей программой помощи с трудоустройством?',
 'Когда я получу удостоверение?',
 'Как узнать, когда у меня будут занятия?',
 'Как добавить инфо о себе в мой аккаунт?',
 'В какие сроки можно пересдать работу, если она не принята?',
 'Каким образом я могу обновить информацию в моем профиле?',
 'Мне интересно, каким образом я могу получить помощь с трудоустройством у вас?',
 'Могу ли я рассчитывать на ваше содействие в трудоустройстве после завершения обучения?\n',
 'Где посмотреть уроки?',
 'Что я получу после обучения?',
 'Мне нужна помощь с трудоустройством. Что нужно для этого?',
 'Какая программа подойдет, если у меня уже есть опыт в ИТ?',
 'Могли бы вы подсказать, какие требования к программному обеспечению нужны для обучения на программисте?',
 'Входит ли в вашу программу помощь студентам с поиском работы после окончания курсов?',
 'Сколько раз можно перевестить в другую группу за весь срок обучения?',
 'Если я сдал все промежуточные аттестации, но не смог пройти итоговую аттестацию, мне все равно выдадут документ об окончании курсов?',
 'Где посмотреть вебинар?',
 'По какому расписанию я буду учиться?',
 'Когда запланированы каникулы на текущий учебный год в GB?',
 'Как часто обновляется расписание курсов?',
 'Какие курсы идут в начале программы?',
 'Помогите найти обучение',
 'Как я могу получить доступ к своим курсам в личном кабинете?',
 'Сколько длится обучение?',
 'Какие даты каникул установлены на этот год в GB?',
 'Какие основные этапы включает в себя программа обучения?',
 'Какие шаги предпринимаются для помощи студентам в получении работы?',
 'Куда приходить на урок?',
 'Хотел бы уточнить, какая именно помощь с трудоустройством предоставляется вашей школой?',
 'Можете ли вы рассказать, как происходит процесс получения поддержки с трудоустройством для студентов вашей школы?',
 'Могу ли я загрузить файл с выполненным заданием?',
 'Могу ли я получить диплом или удовтоверение по почте?',
 'Что будет моим результатом по окончании курса?',
 'Какие требования ПО нужны, если я учусь на тестировщика?',
 'Что делать, если преподаватель поставил «не принято» моей работе?',
 'В чем состоит основное различие программы профессиональной переподготовки по сравнению с программой повышения квалификации?',
 'Какой курс будет следующий?',
 'Я закончил обучение и хочу устроиться на работу, мне говорили, вы помогаете с этим. Как получить эту услугу?',
 'В чем разница между семинаром и практикумом?',
 'Как получить диплом?',
 'Помогаете ли вы найти работу после обучения?',
 'Зачем нужно сдавать итоговую и все промежуточные проверки?',
 'Из каких блоков состоит моя программа?',
 'Хочу получить налоговый вычет. Возможно ли это? Что для этого нужно?',
 'Как пересдать дз?',
 'Что входит в основное обучение в ЛК?',
 'Когда нужно было оплатить обучение, чтобы получить диплом?',
 'Предоставляет ли ваша школа возможность получить налоговый вычет за обучение?',
 'Где проходит вебинар?',
 'Налоговая не приняла ваш договор. Что делать?',
 'Как происходит проверка домашнего задания?',
 'Где найти информацию про документах/сертификаты после окончания обучения?',
 'В какие даты пройдут каникулы в GB в этом году?',
 'Могу ли я получить диплом, если у меня есть среднее профессиональное образование, но нет диплома',
 'Могли бы вы подсказать, какие требования к программному обеспечению нужны для обучения на тестировщике?',
 'Существует ли у вас система поддержки студентов в трудоустройстве после окончания курсов?',
 'В каком формате проходят уроки в GB?',
 'Могу ли я получить удостоверение, если у меня есть среднее профессиональное образование, но нет диплома',
 'Что за вкладка "Отдельные курсы" в ЛК?',
 'При каких условиях я могу получить диплом?',
 'Какой лимит переводов на весь период обучения, если моя программа предусматривает выбор специализации?',
 'Как проходят вебинарные курсы?']

    Нужно для каждого вопроса указать номер наиболее подходящего ответа из предложенных выше.

    Пример:

    Как получить диплом? - 1
    Занятия проходят в Zoom? - 5

    Формируй ответ только в виде вопрос - номер ответа
    """
